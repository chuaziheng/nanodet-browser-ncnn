...
class Trainer:
    ...
    def set_device(self, batch_per_gpu, gpu_ids, device):
        ...
        if num_gpu>=1:
            ...
            self.logger.log('Batch sizes: {}'.format(batch_sizes)) # [2], since we only have 1 gpu
        else:
            self.logger.log("USING CPU ONLY")
            ...
            if iter_id % self.cfg.log.interval == 0:
                ...
                for l in step_losses:
                    ...
                    if mode == 'train' and self.rank < 1:
                        ...
                        self.logger.writer.add_scalar('Train_loss/' + l, step_losses[l].avg(), self._iter)
    ...
    def run(self, train_loader, val_loader, evaluator):
        ...
        for epoch in range(start_epoch, self.cfg.schedule.total_epochs + 1):
            ...
            for k, v in train_loss_dict.items():
                ...
                self.logger.writer.add_scalar("Epoch_train_loss/"+k, v, epoch)
            self.logger.writer.add_scalar("Epoch_train_loss/total", total_loss_train, epoch)
            self.logger.log(f"THE TOTAL TRAINING LOSS FOR THE EPOCH IS {total_loss_train}")
            ...
            if self.cfg.schedule.val_intervals > 0 and epoch % self.cfg.schedule.val_intervals == 0:
                ...
                for k, v in val_loss_dict.items():
                    ...
                    self.logger.writer.add_scalar("Epoch_val_loss/"+k, v, epoch)
                self.logger.writer.add_scalar("Epoch_val_loss/total", total_loss_val, epoch)
                self.logger.log(f"THE TOTAL VALIDATION LOSS FOR THE EPOCH IS {total_loss_val}")
